...
$(window).load(function(){
	var firstInput = $('#ProcessPageEdit input:not([type=submit]):not([type=file]):not([type=checkbox]):not([type=radio]):first-child');
	
	$('.InputfieldTextboxListField').textboxlist({bitsOptions:{editable:{addKeys: [188, 13]}}});
	
	$('.InputfieldTextboxList .textboxlist-bits').css({
		'border-style': firstInput.css('border-style'),
		'border-width': firstInput.css('border-width'),
		'border-color': firstInput.css('border-color'),
		'padding': firstInput.css('padding'),
		'padding-bottom': (parseInt(firstInput.css('padding-bottom'))-3),
		'background': firstInput.css('background'),
		'border-color': firstInput.css('border-color'),
	});	
	
	$('.InputfieldTextboxList .textboxlist-bits input:not([type=submit]):not([type=file]):not([type=checkbox]):not([type=radio])').css({
		'color': firstInput.css('color'),
		'font': firstInput.css('font')
	});	

	// manually adding growing with new style attributes
	$('.InputfieldTextboxList .textboxlist-bit-editable-input').each(function(){		
